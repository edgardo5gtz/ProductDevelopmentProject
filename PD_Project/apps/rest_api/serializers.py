...
class RiskFieldSerializer(serializers.ModelSerializer):
    ...
    number_field = NumberFieldSerializer(many=True)
    enum_field = EnumFieldSerializer(many=True)
    text_field = TextFieldSerializer(many=True)
    date_field = DateFieldSerializer(many=True)
...
class RiskTypeSerializer(serializers.ModelSerializer):
    ...
    risk_field = RiskFieldSerializer(many=True)
...
class RiskSerializer(serializers.ModelSerializer):
    ...
    risk_type = RiskTypeSerializer(many=True)
    ...
    def create(self, validated_data):
        ...
        for risk_type_data in risks_type_data:
            RiskType.objects.create(risk=risk, **risk_type_data)
        return risk